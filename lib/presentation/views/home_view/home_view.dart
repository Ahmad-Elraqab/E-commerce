...
class _HomeViewState extends State<HomeView> {
  ...
  // Clean Architecture - View Model
  late VendorsViewModel _vendorsViewModel;
  ...
  void _initViewModel() {
    _vendorsViewModel = sl<VendorsViewModel>();
    _vendorsViewModel.addListener(_onVendorsUpdated);
    _vendorsViewModel.fetchVendors(limit: 10);
  }

  void _onVendorsUpdated() {
    if (mounted) {
      setState(() {
        _vendors = _vendorsViewModel.vendors;
        _isLoadingVendors = _vendorsViewModel.state == ViewState.loading;
        _vendorsError = _vendorsViewModel.errorMessage;
      });
    }
  }

  Future<void> _fetchVendors() async {
    await _vendorsViewModel.fetchVendors(limit: 10, refresh: true);
  }

  @override
  void dispose() {
    _vendorsViewModel.removeListener(_onVendorsUpdated);
