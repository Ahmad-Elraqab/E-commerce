...
class _CategoriesViewState extends State<CategoriesView> {
  // Live Categories from TuwaTech API
  List<ProductCategoryModel> _apiCategories = [];
  bool _isLoading = true;
  String? _error;

  // Fallback static categories (used when API fails)
  ...
  Future<void> _fetchCategories() async {
    try {
      ...
      final response = await tuwaTechStoreService.getProductCategories(limit: 50);
  ...
  Color _getCategoryColor(ProductCategoryModel category, int index) {
  ...
  IconData _getCategoryIcon(ProductCategoryModel category) {
    ...
    return RefreshIndicator(
      ...
      child: CustomScrollView(
        slivers: [
          ...
          else if (_error != null && _apiCategories.isEmpty)
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate((context, index) {
                  if (_apiCategories.isNotEmpty) {
                    return _buildApiCategoryCard(_apiCategories[index], index);
                  } else {
                    return _buildStaticCategoryCard(_staticCategories[index]);
                  }
                }, childCount: _apiCategories.isNotEmpty ? _apiCategories.length : _staticCategories.length),
              ),
            ),
  ...
  Widget _buildFeaturedBanner() {
    final categoryCount = _apiCategories.isNotEmpty ? _apiCategories.length : _staticCategories.length;
  ...
  Widget _buildApiCategoryCard(ProductCategoryModel category, int index) {
